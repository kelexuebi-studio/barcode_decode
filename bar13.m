...
% num= 0;      %检测59根条形码
i=200;         %200是经验值，表示条形码区域高度的中值，在条形码区域的中间位置进行译码，准确率最高
k=0; 
number=zeros(12,4);   %记录每个数字符号对应的黑白条码的宽度，EAN13码共有13个字符，除了前置码以外（12），其余均有两条黑色条纹和两条白色条纹组成（4）
%%% bar_y 记录的是每个模块灰度突变的位置，以用来确定组成每个字符的黑白模块的宽度（一个字符由7个模块组成，共2黑2白）
bar_y=zeros(1,60);
for j=1:n-1
